...
public class Student implements Runnable {
	...
	@Override
	public void run() {
		Test myTest = new Test(id, new Date());
		try {
			boolean[] answers = solveTheTest();
			myTest.setAnswers(answers);
			studentTest.insert(myTest);
			this.proctorsQueue.insert(this);
			myTest = this.informationSystem.find(id);
	...
	private boolean[] solveTheTest() throws InterruptedException {
		boolean[] answers = new boolean[20];
		for (int i = 0; i<20; i++) {
			Thread.sleep((int)(x*1000.0));
			if (Math.random() <= p) {
				answers[i] = true;
			}
		}
